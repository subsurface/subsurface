		...
		actions: [
			Kirigami.Action {
				...
				onTriggered: {
					...
					if (prefs.credentialStatus == SsrfCloudStoragePrefs.CS_UNKNOWN) {
						// the user has asked to change credentials - if the credentials before that
						// were valid, go back to dive list
						if (oldStatus == SsrfCloudStoragePrefs.CS_VERIFIED) {
			...
			Kirigami.Action {
				...
				Kirigami.Action {
					...
					enabled: prefs.credentialStatus === SsrfCloudStoragePrefs.CS_VERIFIED ||
							prefs.credentialStatus === SsrfCloudStoragePrefs.CS_NOCLOUD
					onTriggered: {
						globalDrawer.close()
				...
				Kirigami.Action {
					...
					enabled: prefs.credentialStatus === SsrfCloudStoragePrefs.CS_VERIFIED ||
							prefs.credentialStatus === SsrfCloudStoragePrefs.CS_NOCLOUD
					onTriggered: {
						if (prefs.credentialStatus === SsrfCloudStoragePrefs.CS_NOCLOUD) {
							returnTopPage()
							oldStatus = prefs.credentialStatus
							manager.startPageText = "Enter valid cloud storage credentials"
							prefs.credentialStatus = SsrfCloudStoragePrefs.CS_UNKNOWN
							globalDrawer.close()
						} else {
							globalDrawer.close()
							detailsWindow.endEditMode()
				...
				text: syncToCloud ? qsTr("Disable auto cloud sync") : qsTr("Enable auto cloud sync")
					enabled: prefs.credentialStatus !== SsrfCloudStoragePrefs.CS_NOCLOUD
					onTriggered: {
