...
Kirigami.ScrollablePage {
	...
	onRefreshingChanged: {
		if (refreshing) {
			if (prefs.credentialStatus === SsrfCloudStoragePrefs.CS_VERIFIED) {
	...
	StartPage {
		id: startPage
		anchors.fill: parent
		opacity: credentialStatus === SsrfCloudStoragePrefs.CS_NOCLOUD ||
									(credentialStatus === SsrfCloudStoragePrefs.CS_VERIFIED) ? 0 : 1
		visible: opacity > 0
		...
		function setupActions() {
			...
			} else if (prefs.credentialStatus === SsrfCloudStoragePrefs.CS_VERIFIED ||
	...
	onBackRequested: {
		if (startPage.visible && diveListView.count > 0 &&
			prefs.credentialStatus !== SsrfCloudStoragePrefs.CS_INCORRECT_USER_PASSWD) {
