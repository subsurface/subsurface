...
TemplatePage {
    ...
    Component.onCompleted: {
        // Default examples
        gasListModel.append({ "mix": "21/0", "pressure": "3000", "amount": "UNLIMITED", "cost": "0.10" });
        gasListModel.append({ "mix": "100/0", "pressure": "3000", "amount": "UNLIMITED", "cost": "0.50" });
        gasListModel.append({ "mix": "0/100", "pressure": "3000", "amount": "UNLIMITED", "cost": "1.00" });
    }
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Controls.Label { text: qsTr("Cylinder"); font.bold: true; Layout.fillWidth: true ; visible: modeSelector.currentIndex === 2 }
                ...
                ListView {
                    ...
                    delegate: RowLayout {
                        ...
                        TemplateComboBox { 
                            ...
                            // Concatenate "UNLIMITED" to the start of the list provided by C++
                            model: [qsTr("UNLIMITED")].concat(manager.cylinderListInit)
                            
                            // Default to index 0 (UNLIMITED) unless specific logic restores state
                            currentIndex: 0 

                            // Save selection to the model so calculations know the cylinder size (or lack thereof)
                            onCurrentTextChanged: {
                                var amt = currentText
                                gasListModel.setProperty(listRowIndex, "amount", amt)
                            }
                        }
                    ...
                    switch (modeSelector.currentIndex) {
                        ...
                        case 2: // Full Blending
                            ...
                            for (var i = 0; i < gasListModel.count; i++) {
                                ...
                                // Explicitly create a JS object. This ensures C++ receives a valid QVariantMap.
                                availableGases.push({
                                    "mix": item.mix,
