...
# call either in the build directory (it will then sign the compiled app in staging)
# or call with a target directory for it to sign
...
xcrun notarytool submit ./Subsurface-$VERSION.dmg --keychain-profile "notarytool-password" --wait
xcrun stapler staple Subsurface-$VERSION.dmg
