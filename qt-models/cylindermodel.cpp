...
QVariant CylindersModel::data(const QModelIndex &index, int role) const
{
	...
	case Qt::ToolTipRole:
		...
		case SENSORS:
			return tr("In order to assign tank pressure sensor data to this cylinder, select the number of the cylinder that you want to move the data from.");
		}
		break;
	}
...
bool CylindersModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
	...
	case SENSORS: {
		...
		if (ok && std::find_if(dc.tank_sensor_mappings.begin(), dc.tank_sensor_mappings.end(), [row, s](const auto &mapping) { return mapping.cylinder_index == row && mapping.sensor_id == s - 1; }) == dc.tank_sensor_mappings.end()) {
