...
{
	divemode_t divemode;
	switch (get_dive_dc_const(d, dcNr)->divemode) {
	case OC:
	default:
		divemode = OC;
...
QVariant DivePlannerPointsModel::data(const QModelIndex &index, int role) const
{
	...
	bool isInappropriateCylinder = !is_cylinder_use_appropriate(get_dive_dc_const(d, dcNr), get_cylinder(d, p.cylinderid), false);
...
Qt::ItemFlags DivePlannerPointsModel::flags(const QModelIndex &index) const
{
	...
	case CCSETPOINT:
		...
		break;
	case DIVEMODE:
		if (!((get_dive_dc_const(d, dcNr)->divemode == CCR && prefs.allowOcGasAsDiluent && get_cylinder(d, p.cylinderid)->cylinder_use == OC_GAS) || get_dive_dc_const(d, dcNr)->divemode == PSCR))
			return QAbstractItemModel::flags(index) & ~Qt::ItemIsEditable & ~Qt::ItemIsEnabled;
