          ...
          <xsl:if test="not(/uddf/diver/owner/equipment/divecomputer/@id = 'INSPIRATION' and position() &gt; 2)">
            <extradata>
              <xsl:attribute name="key">
                <xsl:value-of select="concat('Battery ', position(), ' at Begin')"/>
              </xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="concat($batteryvoltage_values[batteryvoltage[@ref = $reference]][1]/batteryvoltage[@ref = $reference], ' V')"/>
              </xsl:attribute>
            </extradata>
            <extradata>
              <xsl:attribute name="key">
                <xsl:value-of select="concat('Battery ', position(), ' at End')"/>
              </xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="concat($batteryvoltage_values[batteryvoltage[@ref = $reference]][last()]/batteryvoltage[@ref = $reference], ' V')"/>
              </xsl:attribute>
            </extradata>
          </xsl:if>
        </xsl:for-each>
                ...
                <xsl:attribute name="value">
                  <xsl:call-template name="getTempstikGraph">
                    <xsl:with-param name="tempstik_value">
                      <xsl:value-of select="samples/waypoint[scrubber[@ref = 'tempstik']][1]/scrubber[@ref = 'tempstik']"/>
                    </xsl:with-param>
                  </xsl:call-template>
                </xsl:attribute>
              </extradata>
