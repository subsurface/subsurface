...
{
	state->cur_tank_sensor_mapping.sensor_index = 0;
	state->cur_tank_sensor_mapping.cylinder_id = -1;
}

static void tank_sensor_mapping_end(struct parser_state *state)
{
	get_dc(state)->tank_sensor_mappings.push_back(tank_sensor_mapping { state->cur_tank_sensor_mapping.sensor_index, state->cur_tank_sensor_mapping.cylinder_id });
}
...
static int match_dc_data_fields(struct divecomputer *dc, const char *name, char *buf, struct parser_state *state)
{
	...
	if (MATCH("value.extradata", utf8_string_std, &state->cur_extra_data.value))
		return 1;
	if (MATCH("sensorindex.tanksensormapping", get_index, (int *)&state->cur_tank_sensor_mapping.sensor_index))
		return 1;
	if (MATCH("cylinderid.tanksensormapping", get_index, &state->cur_tank_sensor_mapping.cylinder_id))
		return 1;
