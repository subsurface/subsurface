...
namespace Blender {
	...
	double Blend::neededO2() const {
		...
		if( neededVol < 1){
			if(currentO2() > target.targetO2_permille){
				return -10000;
			}
			return 10000;
		}
	...
	double Blend::neededHe() const {
		...
		if( neededVol < 1){
			if(currentHe() > target.targetHe_permille){
				return -10000;
			}
			return 10000;
		}
	...
	bool Blend::correctPressures() {
		...
		for (const BlendStep& step : steps) {
			...
			currentPressure = calculatePressure((double)currentStepO2 / currentVolume, (double)currentStepHe / currentVolume, target.wetVolume.mliter, currentVolume) - ATM;
	...
	bool calculateIntersection(double x1, double y1, double x2, double y2,
		...
		if (std::abs(D) < TIGHT_EPS) {
			// Handle the single-point/collinear case using projection
			double segmentLenSq = dx1 * dx1 + dy1 * dy1;
			if (segmentLenSq < TIGHT_EPS) return false; 

			// Project point (x3, y3) onto line 1
			double t = ((x3 - x1) * dx1 + (y3 - y1) * dy1) / segmentLenSq;
			
			// Is the projection "close enough" to the actual point? (Collinearity check)
			double projX = x1 + t * dx1;
			double projY = y1 + t * dy1;
			double distSq = std::pow(projX - x3, 2) + std::pow(projY - y3, 2);
			
			if (distSq < LOOSE_EPS && t >= -LOOSE_EPS && t <= (1 + LOOSE_EPS)) {
				out_x = x1 + std::max(0.0, std::min(1.0, t)) * dx1;
				out_y = y1 + std::max(0.0, std::min(1.0, t)) * dy1;
				return true;
			}
			return false;
		}

		// General case: intersection of two segments
		double t = ((y3 - y1) * dx2 - (x3 - x1) * dy2) / D;
		double u = ((y3 - y1) * dx1 - (x3 - x1) * dy1) / D;

		// Use a small buffer for the segment bounds
		if (t >= -1e-5 && t <= 1.0 + 1e-5 && u >= -1e-5 && u <= 1.0 + 1e-5) {
			// CLAMP t to [0,1] and reconstruct from Segment 1 ONLY
			// This makes it physically impossible for the point to be "off the line"
	...
	Blend* calculate_blend(const Blend* blend1, const Blend* blend2, const Blend* blend3, const GasSource* source1, const GasSource* source2, const GasSource* source3){
		...
		std::vector<Blend> pointsOfInterest;
				//Ensure all sources are defined, even if we reuse source 1

		if (source2 == nullptr) {
			//Make source2 a copy of source1
			source2 = source1;
		}
		if (source3 == nullptr) {
			//Make source3 a copy of source1
			source3 = source1;
		}
		if (numBlends == 1) {
			...
			if(valid && finalResult->validate(false)) {
				return finalResult;
			} else {
				delete finalResult;
				return nullptr;
