...
static void parse_dc_tanksensormapping(char *line, struct git_parser_state *state)
{
	...
	state->active_dc->tank_sensor_mappings.push_back(tank_sensor_mapping { (unsigned int)atoi(state->converted_strings[0].c_str()), atoi(state->converted_strings[1].c_str()) });
}
