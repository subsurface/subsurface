...
class qPrefCS : public QObject {
	Q_OBJECT
	Q_PROPERTY(QString	base_url
				READ	baseUrl
				WRITE	setBaseUrl
				NOTIFY	baseUrlChanged);
	Q_PROPERTY(bool	 	git_local_only
				READ	gitLocalOnly
				WRITE	setGitLocalOnly
				NOTIFY	gitLocalOnlyChanged);
	Q_PROPERTY(QString	git_url
				READ	gitUrl
				WRITE	setGitUrl
				NOTIFY	gitUrlChanged);
	Q_PROPERTY(QString	email
				READ	email
				WRITE	setEmail
				NOTIFY	emailChanged);
	Q_PROPERTY(QString	email_encoded
				READ	emailEncoded
				WRITE	setEmailEncoded
				NOTIFY	emailEncodedChanged);
	Q_PROPERTY(QString	new_password
				READ	newPassword
				WRITE	setNewPassword
				NOTIFY	newPasswordChanged);
	...
	Q_PROPERTY(QString	pin
				...
				WRITE	setPin
				NOTIFY	pinChanged);
	Q_PROPERTY(short	timeout
				READ	timeout
				WRITE	setTimeout
				NOTIFY	timeoutChanged);
	Q_PROPERTY(bool		save_password_local
				READ	savePasswordLocal
				WRITE	setSavePasswordLocal
				NOTIFY	savePasswordLocalChanged);
	Q_PROPERTY(bool		save_userid_local
				READ	saveUserIdLocal
				WRITE	setSaveUserIdLocal
				NOTIFY	saveUserIdLocalChanged);
	Q_PROPERTY(QString	userid
				READ	userId
				WRITE	setUserId
				NOTIFY	userIdChanged);
	Q_PROPERTY(short	verification_status
				READ	verificationStatus
				WRITE	setVerificationStatus
				NOTIFY	verificationStatusChanged);

public:
	qPrefCS(QObject *parent = NULL) : QObject(parent) {};
	~qPrefCS() {};
	static qPrefCS *instance();
