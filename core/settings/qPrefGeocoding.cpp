...
qPrefGeocoding *qPrefGeocoding::m_instance = NULL;
qPrefGeocoding *qPrefGeocoding::instance()
{
	if (!m_instance)
		m_instance = new qPrefGeocoding;
	return m_instance;
}

void qPrefGeocoding::loadSync(bool doSync)
{
	diskFirstTaxonomyCategory(doSync);
	diskSecondTaxonomyCategory(doSync);
	diskThirdTaxonomyCategory(doSync);
}
