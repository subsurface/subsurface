...
std::pair<QImage, mediatype_t> Thumbnailer::fetchImage(const QString &filename, const QString &originalFilename, bool tryDownload)
{
...
std::pair<QImage, mediatype_t> Thumbnailer::getHashedImage(const QString &filename, bool tryDownload)
{
	QImage thumb;
	...
	if (localFilename != filename)
		std::tie(thumb, type) = fetchImage(localFilename, filename, tryDownload);
	...
	if (thumb.isNull() && type != MEDIATYPE_STILL_LOADING)
		std::tie(thumb, type) = fetchImage(filename, filename, tryDownload);
...
std::pair<QImage, mediatype_t> Thumbnailer::getThumbnailFromCache(const QString &picture_filename)
{
...
static void addThumbnailToCache(const QImage &thumbnail, mediatype_t type, const QString &picture_filename)
{
	if (thumbnail.isNull())
		return;
	...
	stream << (quint32)type;
	if (type == MEDIATYPE_PICTURE) // TODO: Also support caching of video thumbnails
		stream << thumbnail;
	file.commit();
}
...
void Thumbnailer::recalculate(QString filename)
{
	QImage thumbnail;
	mediatype_t type;
	std::tie(thumbnail, type) = getHashedImage(filename, true);
	...
	if (type == MEDIATYPE_STILL_LOADING || thumbnail.isNull())
		return;
	addThumbnailToCache(thumbnail, type, filename);

	QMutexLocker l(&lock);
	emit thumbnailChanged(filename, thumbnail, type);
...
void Thumbnailer::processItem(QString filename, bool tryDownload)
{
	QImage thumbnail;
	mediatype_t type;
	std::tie(thumbnail, type) = getThumbnailFromCache(filename);

	if (thumbnail.isNull()) {
		std::tie(thumbnail, type) = getHashedImage(filename, tryDownload);
		if (type == MEDIATYPE_STILL_LOADING)
			return;
