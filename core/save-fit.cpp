 ...
 * encoder. None of this comes from the Garmin FIT SDK (R009).
 */
...
// Enum values from the (non-Garmin) Go SDK FIT profile, per R009.
...
uint16_t fit_manufacturer_id(const std::string &vendor)
{
	...
	// not the Garmin FIT SDK (R009). Most Subsurface-supported vendors have
...
int fit_resolve_tz_offset(const struct divecomputer &dc)
{
	...
	// wants (T04/T05: CEST dives were previously pre-filled with dc.timezone_offset
	// or the device's local UTC offset and exported hours off). The dc argument
